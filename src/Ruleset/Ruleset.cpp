 ...
 */
Ruleset::Ruleset() : _costSoldier(0), _costEngineer(0), _costScientist(0), _timePersonnel(0), _initialFundAdjust(0), _startingTime(6, 1, 1, 1999, 12, 0, 0), _modIndex(0), _facilityListOrder(0), _craftListOrder(0), _itemListOrder(0), _researchListOrder(0),  _manufactureListOrder(0), _ufopaediaListOrder(0)
{
...
void Ruleset::loadFile(const std::string &filename)
{
	...
	if (doc["startingBase"])
 	...
 	_initialFundAdjust = doc["initialFundAdjust"].as<int>(_initialFundAdjust);
...
SavedGame *Ruleset::newSave() const
{
	...
	int missing = ((_initialFundAdjust - save->getCountryFunding()/1000) / (int)save->getCountries()->size()) * 1000;
