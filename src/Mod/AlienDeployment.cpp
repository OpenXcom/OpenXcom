 ...
 */
AlienDeployment::AlienDeployment(const std::string &type) : _type(type), _width(0), _length(0), _height(0), _civilians(0), _shade(-1), _finalDestination(false), _isAlienBase(false), _isEscapeMission(false), _alert("STR_ALIENS_TERRORISE"),
...
void AlienDeployment::load(const YAML::Node &node, Mod *mod)
{
	...
	_isEscapeMission = node["escapeMission"].as<bool>(_isEscapeMission);
	if (node["genMission"])
	{
...
bool AlienDeployment::isEscapeMission() const
{
	return _isEscapeMission;
}
