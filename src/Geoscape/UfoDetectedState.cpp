...
UfoDetectedState::UfoDetectedState(Ufo *ufo, GeoscapeState *state, bool detected, bool hyperwave) : _ufo(ufo), _state(state), _hyperwave(hyperwave)
{
	...
	_btnIntercept = new TextButton(200, 12, 28, _game->getLanguage()->getWindowValue(118, "UfoDetectedInterceptY"));
	_btnCentre = new TextButton(200, 12, 28, _game->getLanguage()->getWindowValue(134, "UfoDetectedCentreY"));
	_btnCancel = new TextButton(200, 12, 28, _game->getLanguage()->getWindowValue(150, "UfoDetectedCancelY"));
	_txtUfo = new Text(207, 17, 28, 53);
	_txtDetected = new Text(100, 9, 28, 69);
	_txtHyperwave = new Text(214, 17, 21, 44);
	_lstInfo = new TextList(207, _game->getLanguage()->getWindowValue(32, "UfoDetectedInfoH"), 28, 80);
	_lstInfo2 = new TextList(207, _game->getLanguage()->getWindowValue(32, "UfoDetectedInfo2H"), 28, 96);
