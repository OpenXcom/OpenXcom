 ...
 */
GeoscapeState::GeoscapeState() :  _pause(false), _music(false), _zoomInEffectDone(false), _zoomOutEffectDone(false), _battleMusic(false), _popups(), _dogfights(), _dogfightsToBeStarted(), _minimizedDogfights(0)
{
...
void GeoscapeState::handle(Action *action)
{
	...
	if (action->getDetails()->type == SDL_KEYDOWN)
	{
		...
		// quick save and quick load
            ...
            _game->pushState(new SaveState( OPT_GEOSCAPE, true));
		else if (action->getDetails()->key.keysym.sym == Options::getInt("keyQuickLoad") && Options::getInt("autosave") == 1)
            _game->pushState(new LoadState( OPT_GEOSCAPE, true));
	}
	if(!_dogfights.empty())
	{
		for(std::vector<DogfightState*>::iterator it = _dogfights.begin(); it != _dogfights.end(); ++it)
					...
					case Ufo::DESTROYED: // Just before expiration
						if ((*j)->getNumSoldiers() > 0 || (*j)->getNumVehicles() > 0)
						{
							if(!(*j)->isInDogfight())
							{
								...
								timerReset();
                                popup(new ConfirmLandingState( *j, texture, shade, this));
							}
						}
...
void GeoscapeState::time1Day()
{
	...
	if (Options::getInt("autosave") >= 2)
        _game->pushState(new SaveState( OPT_GEOSCAPE, false));
}
...
void GeoscapeState::btnBasesClick(Action *)
{
	...
	if (!_game->getSavedGame()->getBases()->empty())
	{
        _game->pushState(new BasescapeState( _game->getSavedGame()->getBases()->front(), _globe));
	}
	else
	{
...
void GeoscapeState::handleBaseDefense(Base *base, Ufo *ufo)
{
	...
	if (base->getAvailableSoldiers(true) > 0)
	{
		...
		bgen.run();
		musicStop();
        popup(new BriefingState( 0, base));
	}
	else
	{
