 ...
 */
MonthlyReportState::MonthlyReportState(Game *game, bool psi) : State(game), _psi(psi), _ratingTotal(0), _fundingDiff(0), _generalSatisfaction(0), _pactList(0), _happyList(0), _sadList(0)
{
	// Create objects
	...
	{
	case 0:
		ss4 << _game->getLanguage()->getString("STR_YOU_HAVE_NOT_SUCCEEDED").c_str();
		// game over man
		break;
	case 1:
		ss4 << _game->getLanguage()->getString("STR_COUNCIL_IS_DISSATISFIED").c_str();
		break;
	case 3:
		ss4 << _game->getLanguage()->getString("STR_COUNCIL_IS_VERY_PLEASED").c_str();
		break;
	default:
		ss4 << _game->getLanguage()->getString("STR_COUNCIL_IS_GENERALLY_SATISFIED").c_str();
		break;
	}
	if(_happyList.size())
	{
		...
		{
			ss4 << _game->getLanguage()->getString(*happy).c_str();

			if(happy == _happyList.end()-2 && _happyList.size() > 1)
			{
				ss4 << _game->getLanguage()->getString("STR_AND").c_str();
			}
			if(happy != _happyList.end() - 1 && happy != _happyList.end() - 2 && _happyList.size() > 3)
			{
				ss4 << ", ";
			}
			if(happy == _happyList.end()-1)
			{
				...
				{
					ss4 << _game->getLanguage()->getString("STR_COUNTRIES_ARE_PARTICULARLY_HAPPY").c_str();	
				}
				else
				{
					ss4 << _game->getLanguage()->getString("STR_COUNTRY_IS_PARTICULARLY_PLEASED").c_str();
				}
			}
		}
	}
	if(_sadList.size())
	{
		...
		{
			ss4 << _game->getLanguage()->getString(*sad).c_str();

			if(sad == _sadList.end()-2 && _sadList.size() > 1)
			{
				ss4 << _game->getLanguage()->getString("STR_AND").c_str();
			}
			if(sad != _sadList.end() - 1 && sad != _sadList.end() - 2 && _sadList.size() > 3)
			{
				ss4 << ", ";
			}
			if(sad == _sadList.end()-1)
			{
				if(_sadList.size() > 1)
				{
					ss4 << _game->getLanguage()->getString("STR_COUNTRIES_ARE_UNHAPPY_WITH_YOUR_ABILITY").c_str();	
				}
				else
				{
					ss4 << _game->getLanguage()->getString("STR_COUNTRY_IS_UNHAPPY_WITH_YOUR_ABILITY").c_str();
				}
			}
		}
	}
	if(_pactList.size())
	{
		...
		{
			ss4 << _game->getLanguage()->getString(*pact).c_str();

			if(pact == _pactList.end()-2 && _pactList.size() > 1)
			{
				ss4 << _game->getLanguage()->getString("STR_AND").c_str();
			}
			if( pact != _pactList.end()-1 && pact != _pactList.end()-2 && _pactList.size() > 3)
			{
				ss4 << ", ";
			}
			if(pact == _pactList.end()-1)
			{
				if(_pactList.size() > 1)
				{
					ss4 << _game->getLanguage()->getString("STR_COUNTRIES_HAVE_SIGNED_A_SECRET_PACT").c_str();	
				}
				else
				{
					ss4 << _game->getLanguage()->getString("STR_COUNTRY_HAS_SIGNED_A_SECRET_PACT").c_str();
				}
			}
		}
	}
