...
MonthlyCostsState::MonthlyCostsState(Game *game, Base *base) : State(game), _base(base)
{
	...
	for (std::vector<std::string>::iterator i = crafts.begin(); i != crafts.end(); ++i)
	{
		...
		if (craft->getBuyCost() > 0)
		{
			...
			_lstCrafts->addRow(4, tr(*i).c_str(), Text::formatFunding(craft->getBuyCost()).c_str(), ss2.str().c_str(), Text::formatFunding(_base->getCraftCount(*i) * craft->getBuyCost()).c_str());
		}
	}
