...
void PlaceFacilityState::viewClick(Action *)
{
	...
	{
		_game->popState();
        _game->pushState(new ErrorMessageState( "STR_CANNOT_BUILD_HERE", Palette::blockOffset(15)+1, "BACK01.SCR", 6));
	}
	else if (_game->getSavedGame()->getFunds() < _rule->getBuildCost())
	{
		_game->popState();
        _game->pushState(new ErrorMessageState( "STR_NOT_ENOUGH_MONEY", Palette::blockOffset(15)+1, "BACK01.SCR", 6));
	}
	else
	{
