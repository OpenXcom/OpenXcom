...
void ManufactureInfoState::handleWheelEngineer(Action *action)
{
	...
	{
		if (action->getDetails()->button.x > 0) moreEngineer(Options::changeValueByMouseWheel);
		else if (action->getDetails()->button.x < 0) lessEngineer(Options::changeValueByMouseWheel);
	}
}
...
void ManufactureInfoState::handleWheelUnit(Action *action)
{
	if (action->getDetails()->type == SDL_MOUSEWHEEL)
	{
		if (action->getDetails()->button.x > 0) moreUnit(Options::changeValueByMouseWheel);
		else if (action->getDetails()->button.x < 0) lessUnit(Options::changeValueByMouseWheel);
	}
}
