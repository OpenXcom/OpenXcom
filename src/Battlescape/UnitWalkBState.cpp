...
void UnitWalkBState::playMovementSound()
{
	...
	if (_unit->getMoveSound() != -1)
	{
		...
		{
			_parent->getRuleset()->getSoundByDepth(_parent->getDepth(), _unit->getMoveSound())->play(-1, _parent->getMap()->getSoundAngle(_unit->getPosition()));
		}
	}
	else
	{
		if (_unit->getStatus() == STATUS_WALKING)
		{
			...
			if (_unit->getWalkingPhase() == 3)
			{
				...
				{
					_parent->getRuleset()->getSoundByDepth(_parent->getDepth(), Ruleset::WALK_OFFSET + (tile->getFootstepSound(tileBelow)*2))->play(-1, _parent->getMap()->getSoundAngle(_unit->getPosition()));
				}
			}
			...
			if (_unit->getWalkingPhase() == 7)
			{
				if (tile->getFootstepSound(tileBelow) > -1)
				{
					_parent->getRuleset()->getSoundByDepth(_parent->getDepth(), 1 + Ruleset::WALK_OFFSET + (tile->getFootstepSound(tileBelow)*2))->play(-1, _parent->getMap()->getSoundAngle(_unit->getPosition()));
				}
			}
		}
		else
		{
			...
			{
				_parent->getRuleset()->getSoundByDepth(_parent->getDepth(), Ruleset::FLYING_SOUND)->play(-1, _parent->getMap()->getSoundAngle(_unit->getPosition()));
			}
		}
	}
}
